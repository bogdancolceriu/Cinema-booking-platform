...
const Genres = ({ setGenreIds }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedGenres = await FetchGenres(ACCESS_TOKEN);
        console.log('Fetched genres:', fetchedGenres); // Debugging: Verifică ce returnează FetchGenres
        const filteredGenres = RemoveUnwantedGenres(fetchedGenres);
        console.log('Filtered genres:', filteredGenres); // Debugging: Verifică rezultatul după filtrare
        setGenres(filteredGenres);
        setClickedGenres(Array(filteredGenres.length).fill(false));
      } catch (error) {
